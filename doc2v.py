...
sample_corpus=[]
with open('./doc2v/교양_불용어처리+수업코드+내용_0527.csv','r', newline='') as f:
    rdr=csv.reader(f)
    for row in rdr:
...
raw_corpus = pd.read_csv('./doc2v/교양_불용어처리+수업코드+내용_0527.csv', encoding = 'UTF8')


raw_corpus.to_csv('./doc2v/교양_불용어처리+수업코드+내용_0527.csv', columns = ['1','수업코드','text','교과목명'], index = False)
...
with open('./doc2v/교양_인덱스.csv', mode='r',encoding='utf-8-sig') as inp:
    reader = csv.reader(inp)
    dict_from_csv = {rows[1]:rows[0] for rows in reader}

print(dict_from_csv['20187'])


from gensim.models.doc2vec import Doc2Vec

def elec_doc_sim(lec_num):
    print("-------------dfaasfads")
    model= Doc2Vec.load("d2v_elec.model")
